...
transactionsRouter.post('/', authMiddleware, async (req, res) => {
    try {
        const { items, paid, paymentMethod, payments, // Array of { method, amount, reference? } for split payment
        customerId, discount = 0, redeemPoints = 0, outletId, } = req.body;
        ...
        if (redeemPoints && redeemPoints > 0) {
            if (!customerId)
                return res.status(400).json({ error: 'Customer required for point redemption' });

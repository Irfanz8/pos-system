    ...
    try {
        ...
        const productsWithStock = products.map((p) => {
            ...
            if (req.query.outletId) {
                const outletStock = p.stocks.find((s) => s.outletId === req.query.outletId);
                stock = outletStock ? outletStock.stock : 0;
            }
            else {
                stock = p.stocks.reduce((acc, s) => acc + s.stock, 0);
            }
            return { ...p, stock };
        });
...
productsRouter.get('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const productWithStock = {
            ...product,
            stock: product.stocks.reduce((acc, s) => acc + s.stock, 0)
        };
...
productsRouter.post('/', authMiddleware, adminOnly, async (req, res) => {
    try {
...
productsRouter.put('/:id', authMiddleware, adminOnly, async (req, res) => {
    try {
